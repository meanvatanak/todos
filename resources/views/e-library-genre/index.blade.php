    ...
    <div class="modal-dialog">
      <div class="modal-content" id="modal-content">
        <div class="modal-header bg-primary">
          <h4 class="modal-title text-light" id="standard-modalLabel">Create Genre</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
        <form method="POST" action="{{ route('genre.store') }}" id="create_store" autocomplete="off">
          @csrf
          <div id="form-content">
    ...
    <div class="modal-dialog">
      <div class="modal-content" id="update-content">
        <div class="modal-header bg-info">
          <h4 class="modal-title text-light" id="standard-modalLabel">Update Genre</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
        <form method="POST" action="{{ route('genre.update') }}" id="update_store" autocomplete="off">
          @csrf
  ...
  <div id="show-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content" id="show-content">
        <div class="modal-header bg-info">
          <h4 class="modal-title text-light" id="standard-modalLabel">Show Genre</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
...
<script>
  $(document).ready(function() {
    ...
    $('#btn_new').on('click', function(e){
      ...
      var widget_url = "<?php echo URL::to('genre/create'); ?>";
      $.ajax({
        type: 'GET',
        url: widget_url,
        // data: { 'index': act },
    ...
    $("#create_store").on('submit',function(e) {
      ...
      if(name_genre == '')
      {
        $('#name_genre').addClass('border-danger');
        $('#name_genre').focus();
    ...
    table.on('draw.dt', function () {
      ...
      $('.edit').on('click', function (e) {
        ...
        var widget_url = "<?php echo URL::to('genre/edit'); ?>";
        $.ajax({
          type: 'GET',
          url: widget_url,
