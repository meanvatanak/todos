...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div class="d-none d-inline-block">
      ...
      @if(isset(session('permissions')[73]) && session('permissions')[73]['optCreate'] != 0)
    ...
    <div class="modal-dialog">
      <div class="modal-content" id="modal-content">
        <div class="modal-header bg-primary">
          <h4 class="modal-title text-light" id="standard-modalLabel">Create Publisher</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
        <form method="POST" action="{{ route('publisher.store') }}" id="create_store" autocomplete="off">
          @csrf
          <div id="form-content">
    ...
    <div class="modal-dialog">
      <div class="modal-content" id="update-content">
        <div class="modal-header bg-info">
          <h4 class="modal-title text-light" id="standard-modalLabel">Update Publisher</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
        <form method="POST" action="{{ route('publisher.update') }}" id="update_store" autocomplete="off">
          @csrf
  ...
  <div id="show-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content" id="show-content">
        <div class="modal-header bg-info">
          <h4 class="modal-title text-light" id="standard-modalLabel">Show Publisher</h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
        </div>
  ...
  $(document).ready(function() {
    ...
    function load_data()
    {
      ...
      table = $('#dataTable2').DataTable({
        ...
        ajax: {
          type: 'GET',
          url: "{{ route('publisher.getDataTable') }}",
          data: {
            'name': name,
            },
        },
    ...
    $('#btn_new').on('click', function(e){
      ...
      var widget_url = "<?php echo URL::to('publisher/create'); ?>";
      $.ajax({
        type: 'GET',
        url: widget_url,
        // data: { 'index': act },
    ...
    $("#create_store").on('submit',function(e) {
      ...
      var name_publisher = $('#name_publisher').val();
      
      if(name_publisher == '')
      {
        $('#name_publisher').addClass('border-danger');
        $('#name_publisher').focus();
    ...
    table.on('draw.dt', function () {
      ...
      $('.edit').on('click', function (e) {
        ...
        var widget_url = "<?php echo URL::to('publisher/edit'); ?>";
        $.ajax({
          type: 'GET',
          url: widget_url,
